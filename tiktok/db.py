        ...
        for db_data in tiktok_data:
            ...
            if c.rowcount == 0:
                c.execute(f"""
                    ...
                    VALUES ('{db_data}')
                    ON CONFLICT ({str(header[0])}) DO UPDATE SET {str(header[1])} = (SELECT {str(header[1])} FROM {db_name} where {str(header[0])} = '{db_data}')  + 1;
                """)
...
db_ = "video_editor_hashtag"
# print(create_table(table_row, 'new_table2'))
print(insert_data(header, dd, db_))
